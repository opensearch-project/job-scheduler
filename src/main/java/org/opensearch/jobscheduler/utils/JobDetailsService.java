...
public final class JobDetailsService {
    ...
    void createJobDetailsIndex(ActionListener<Boolean> listener) {
        if (jobDetailsIndexExist()) {
            ...
            final CreateIndexRequest request = new CreateIndexRequest(JOB_DETAILS_INDEX_NAME).mapping(jobDetailsMapping());
            client.admin()
                .indices()
    ...
    private String jobDetailsMapping() {
        try {
            InputStream in = JobDetailsService.class.getResourceAsStream("opensearch_plugins_job_details.json");
