...
public class RestGetJobTypeActionTests extends OpenSearchTestCase {
    ...
    public void testGetNames() {
        ...
        Assert.assertEquals(action.GET_JOB_TYPE_ACTION, name);
    }

    public void testGetRoutes() {
        List<RestHandler.Route> routes = action.routes();

        Assert.assertEquals(String.format(Locale.ROOT, "%s/%s", JobSchedulerPlugin.JS_BASE_URI, "_get/_job_type"), routes.get(0).getPath());
    }
